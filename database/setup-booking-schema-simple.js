...
async function setupBookingSchema() {
  ...
  try {
    ...
    const createProperty = async (className, propertyName, propertyType) => {
      try {
        await db.query(`CREATE PROPERTY ${className}.${propertyName} ${propertyType}`);
      } catch (error) {
        if (!error.message.includes('already exists')) {
    ...
    const createIndex = async (className, propertyName, indexType = 'NOTUNIQUE') => {
      ...
      try {
        await db.query(`CREATE INDEX ${indexName} ON ${className} (${propertyName}) ${indexType}`);
    ...
    try {
      await db.query('CREATE INDEX Booking.date_status ON Booking (date, status) NOTUNIQUE');
    ...
    try {
      await db.query('CREATE INDEX TimeSlot.date_time ON TimeSlot (date, startTime) UNIQUE');
    ...
    const existingSettings = await db.query('SELECT count(*) as count FROM BookingSettings');
    if (existingSettings[0].count === 0) {
      const settings = [
        { key: 'workingHoursStart', value: '09:00', desc: 'Start of working hours' },
        { key: 'workingHoursEnd', value: '18:00', desc: 'End of working hours' },
      ...
      for (const setting of settings) {
        await db.query(`INSERT INTO BookingSettings SET settingKey = '${setting.key}', settingValue = '${setting.value}', description = '${setting.desc}'`);
      }
